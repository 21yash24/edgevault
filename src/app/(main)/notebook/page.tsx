...
import { useNotebookStore, NotebookNote, NotebookFolder, NotebookTag } from "@/stores";
import { 
...
export default function NotebookPage() {
  const { folders, tags, notes, templates, saveNote, deleteNote, addFolder, addTag } = useNotebookStore();
    ...
    <div className="h-[calc(100vh-80px)] flex flex-col sm:flex-row bg-bg-base border border-border-subtle rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="w-full sm:w-64 bg-bg-card border-r border-border-subtle flex flex-col">
        <div className="p-4 border-b border-border-subtle">
          <h2 className="font-[family-name:var(--font-inter)] font-black text-lg text-text-primary">Notebook</h2>
        </div>
...
function NoteEditor({ noteId }: { noteId: string }) {
  const { notes, saveNote, deleteNote, folders, tags } = useNotebookStore();
